...
function kube::codegen::internal::git_find() {
    ...
    # echo "----git_find:git ls-files -cmo --exclude-standard "$@""
...
function kube::codegen::internal::git_grep() {
    ...
    # echo "----git_grep:git grep --untracked "$@""
    git grep --untracked "$@"
}
...
function kube::codegen::gen_helpers() {
    ...
    if [ "${#input_pkgs[@]}" != 0 ]; then
        ...
        echo "git_find "
        echo kube::codegen::internal::git_find -z \
